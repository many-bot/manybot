...
import type { Message, GroupChat, Chat }        from "#wwjs";
...
function _matches(chat: { name?: string; id?: { user?: string; _serialized?: string } }, term: string): boolean {
...
function _buildRow(chat: { name?: string; id?: { user?: string; _serialized?: string }; isGroup: boolean }) {
  return { name: chat.name || chat.id?.user || "", number: chat.id?.user || "",
           id: chat.id?._serialized || "", group: chat.isGroup };
}

function _printTable(rows: Array<{ name: string; number: string; id: string; group: boolean; phone?: string | null }>): void {
...
function _exportResults(rows: Array<Record<string, unknown>>, csvHeader = "name,number,id,group"): void {
  if (_exportJson) {
  ...
  if (_exportCsv) {
    ...
    fs.writeFileSync(f, [csvHeader,
      ...rows.map((r: Record<string, unknown>) => keys.map((k: string) => `"${String(r[k] ?? "").replace(/"/g, '""')}"`).join(","))
    ].join("\n"));
...
process.on("unhandledRejection", (reason) => {
  const msg = reason instanceof Error ? (reason as Error).message : String(reason);
...
client.on("loading_screen", (p: number, msg: string) => {
  setState("SYNC");
...
client.on("ready", async () => {
  ...
  if (GET_ID_MODE) {
    ...
    if (first === "communities") {
      const communities = chats.filter((c: Chat) => (c as GroupChat).isCommunity);
      ...
      const chatIndex = new Map(chats.map((c: Chat) => [c.id?._serialized, c]));
      ...
      for (const community of communities) {
        ...
        const meta         = (community as GroupChat).groupMetadata;
        const cachedIds    = new Set(chats.map((c: Chat) => c.id?._serialized));

        // Collect all known subgroup IDs from metadata (field name varies by wwebjs version)
        const knownIds = new Set([
          ...(meta?.subGroupsId     || []).map((id: unknown) => (id as { _serialized?: string })?._serialized ?? id as string),
          ...(meta?.linkedSubgroups || []).map((id: unknown) => (id as { _serialized?: string })?._serialized ?? id as string),
          ...(meta?.subgroups       || []).map((id: unknown) => (id as { _serialized?: string })?._serialized ?? id as string),
        ].filter(Boolean) as string[]);

        // Fetch any subgroup not already in getChats()
        const fetched = await Promise.all(
          [...knownIds]
            .filter((id: string) => !chatIndex.has(id))
            .map((id: string) => client.getChatById(id).catch(() => null))
        );
        fetched.filter((c: Chat | null): c is Chat => c !== null).forEach((c: Chat) => chatIndex.set(c.id?._serialized, c));

        // Build final subgroup list: parentGroup match OR in knownIds
        const subgroups = [...chatIndex.values()].filter((c: Chat) =>
          c.isGroup &&
          !(c as GroupChat).isCommunity &&
          ((c as GroupChat).groupMetadata?.parentGroup?._serialized === communityId ||
           knownIds.has(c.id?._serialized))
        );

        // Sort: default subgroup first, then alphabetical
        subgroups.sort((a: Chat, b: Chat) => {
          const aDefault = (a as GroupChat).groupMetadata?.isDefaultSubgroup ?? false;
          const bDefault = (b as GroupChat).groupMetadata?.isDefaultSubgroup ?? false;
        ...
        subgroups.forEach((sg: Chat, i: number) => {
          const isLast    = i === subgroups.length - 1;
          const branch    = isLast ? "└─" : "├─";
          const isDefault = (sg as GroupChat).groupMetadata?.isDefaultSubgroup ?? false;
        ...
        exportRows.push(...subgroups.map((sg: Chat) => ({
          community:   community.name,
          communityId,
          name:        sg.name,
          number:      sg.id?.user || "",
          id:          sg.id?._serialized || "",
          default:     (sg as GroupChat).groupMetadata?.isDefaultSubgroup ?? false,
        })));
      }
    ...
    let filtered;
    if      (first === "groups")   filtered = chats.filter((c: Chat) => c.isGroup);
    else if (first === "contacts") filtered = chats.filter((c: Chat) => !c.isGroup);
    else                           filtered = chats.filter((c: Chat) => _terms.every(term => _matches(c, term)));
      ...
      if (lidRows.length) {
        try {
          const phoneMap = await client.pupPage!.evaluate((ids: string[]) => {
            ...
            return result;
          }, lidRows.map((r: { id: string }) => r.id));
          for (const row of lidRows) (row as Record<string, unknown>).phone = (phoneMap as Record<string, unknown>)[row.id] || null;
