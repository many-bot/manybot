...
const bins: Partial<Record<NodeJS.Platform, string>> = {
  linux: "chrome",
  win32: "chrome.exe"
}
...
if (!fs.existsSync(chrome)) {
  try {
    ...
    const urls: Partial<Record<NodeJS.Platform, string>> = {
...
// -- Qr Handle --------------------------------------------------
export function handleQR(qr: string): void {
  logger.info(t("system.qrScan"));
  qrcode.generate(qr, { small: true });
}

// -- Handle pairing code ---------------------------------------
export function handlePairingCode(code: string): void {
...
function isValidPhoneNumber(phone: unknown): phone is string {
  if (!phone || typeof phone !== "string") return false;
  return /^\d{10,15}$/.test(phone);
}

// Checks if phone number changed since last authentication
function hasPhoneNumberChanged(currentPhone: string): boolean {
  try {
    if (!fs.existsSync(AUTH_STATE_PATH)) return false;
    const state = JSON.parse(fs.readFileSync(AUTH_STATE_PATH, "utf8")) as { phoneNumber?: string };
...
function savePhoneNumber(phone: string): void {
  try {
...
if (PHONE_NUMBER) {
  if (isValidPhoneNumber(PHONE_NUMBER)) {
    (clientOptions as Record<string, unknown>).pairWithPhoneNumber = {
