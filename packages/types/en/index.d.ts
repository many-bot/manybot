 ...
 * `decryptPollVote`, `aggregatePollVotes`, `communityParticipantsUpdate`)
 * are driver-specific extensions; callers MUST handle their absence.
 */
export interface WaContract {
   ...
   * (the parent group) — WhatsApp uses a distinct protocol operation
   * there. Optional — drivers without Community support omit it.
   */
...
export interface WAMessageContext {
   ...
   * @param opts - `asMp4`: convert animated stickers to mp4. `asFrames`: return animated
   * stickers as individual decoded frames (with per-frame delay) instead of the raw webp.
   * @returns The media (base64 `data`, or `frames` when `asFrames` was used) with its
   * `mimetype` and `isAnimated` flag, or `null` if there's no media or the download failed.
   */
   ...
   * @param emoji - A single emoji character, e.g. `"👍"`.
   */
  react(emoji: string): Promise<unknown>;
  /**
...
export interface ChatContext {
   ...
   * bound to the chat this instance is about, this fetches a chat the
   * plugin isn't currently handling a message from (e.g. one discovered
   * via {@link ChatsApi.all} or stored earlier).
   *
   * `isSenderAdmin()` on the returned chat still refers to the sender of
   ...
   * @example
   * ```js
   * const other = await ctx.chat.getChat("123456789@g.us");
   * if (other) console.log(other.name, await other.getParticipants());
   * ```
   */
   ...
   * `.react()`, etc. work on an old/stored message ID just like they
...
export interface UtilsApi {
  ...
  /** Animated-WebP helpers, so plugins don't need to depend on `node-webpmux` directly. */
  webp: {
    /** Whether a WebP buffer/path is an animated (multi-frame) WebP. */
    isAnimated(input: Buffer | string): Promise<boolean>;
    /** Splits an animated WebP into its individual frames, each with its display delay. */
...
export interface PluginContext extends BaseApi {
  ...
  wa: {
    ...
    msg: BotMessage;
    /** Download the current message's media; `asMp4` converts animated stickers to mp4, `asFrames` returns decoded frames + delays. */
    downloadMedia(opts?: { asMp4?: boolean; asFrames?: boolean }): Promise<DownloadedMedia | null>;
  } | null;
